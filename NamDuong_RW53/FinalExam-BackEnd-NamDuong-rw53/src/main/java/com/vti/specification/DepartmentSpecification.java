...
public class DepartmentSpecification {
    public static Specification<Department> buildWhere(DepartmentFilterForm form) {
        return hasUsernameLike(form.getSearch())
                .or(hasDepartmentName(form.getSearch()))
...
//            return builder.like(join.get(Account_.username), "%" + value.trim() + "%");
    ...
    public static Specification<Department> hasDepartmentName(String value) {
        return (root, query, builder) -> {
            if (value == null) {
                return null;
            }
            return builder.like(root.get(Department_.name),"%" + value.trim() + "%");
        };
    }
